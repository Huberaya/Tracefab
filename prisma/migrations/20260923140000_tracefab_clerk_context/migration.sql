-- Replace the historical Supabase JWT email lookup with the Neon/Clerk
-- transaction context before invitation acceptance is exposed by the API.

CREATE OR REPLACE FUNCTION tracefab_current_user_email()
RETURNS TEXT
LANGUAGE sql
STABLE
AS $$
  SELECT NULLIF(current_setting('tracefab.user_email', true), '');
$$;

-- The full replacement is generated below from the historical function. It is
-- kept in this incremental migration so the already-applied initial migration
-- checksum remains immutable.
